...
module.exports = async function globalSetup() {
	...
	try {
		...
		await browser.close();
		throw new Error('[audiocheck:e2e] global-setup: login form not ready with E2E_USER set');
	}
	...
	try {
		...
		await browser.close();
		throw new Error('[audiocheck:e2e] global-setup: login failed with E2E_USER set');
	}
	await context.storageState({ path: outputPath });
	await browser.close();
	console.log(`[audiocheck:e2e] storage state written: ${outputPath}`);
