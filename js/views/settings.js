(function () {
	...
	AudioCheckRouter.register('settings', {
		render() {
			...
			const body = C.el('div', { className: 'ac-page-body ac-settings-page ac-page-stack' });

			const form = C.el('form', {
				className: 'ac-form',
				attrs: { 'data-ac-autosave': 'prefs', novalidate: 'novalidate' },
			});

			const status = C.el('p', {
				className: 'ac-autosave-status',
				attrs: {
					id: 'ac-prefs-autosave',
					role: 'status',
					'aria-live': 'polite',
					'data-state': 'idle',
				},
				text: t('audiocheck', 'Changes save automatically.'),
			});
			...
			const speed = C.el('select', { id: 'ac-default-speed', className: 'ac-input', attrs: { 'aria-describedby': 'ac-speed-hint ac-prefs-autosave' } });
			...
			speedRow.appendChild(C.el('p', {
				id: 'ac-speed-hint',
				className: 'ac-field__hint',
				text: t('audiocheck', 'Used when AudioCheck starts with no saved queue. Changing speed while listening applies to the whole queue.'),
			}));
			...
			const volumeSlider = C.el('input', {
				...
				attrs: {
					...
					max: '100',
					value: '100',
					'aria-describedby': 'ac-volume-hint ac-prefs-autosave',
			...
			const resume = C.el('input', {
				id: 'ac-resume-on-open',
				type: 'checkbox',
				attrs: { 'aria-describedby': 'ac-resume-hint ac-prefs-autosave' },
			});
			...
			const listenedRow = C.el('div', {
				...
				attrs: { role: 'radiogroup', 'aria-label': t('audiocheck', 'Listened threshold'), 'aria-describedby': 'ac-prefs-autosave' },
			});
			...
			const subfolders = C.el('input', {
				id: 'ac-scan-subfolders',
				type: 'checkbox',
				attrs: { 'aria-describedby': 'ac-prefs-autosave' },
			});
			...
			subRow.appendChild(C.el('p', {
				className: 'ac-field__hint',
				text: t('audiocheck', 'Include audio in every nested folder — for example Author/Book/Chapter. Applies to new library folders and the default folder.'),
			}));
			...
			function persistPrefs() {
				if (!hydrated || !form.isConnected) {
					return;
				}
				const seq = ++saveSeq;
				const payload = collectPrefs();
				setStatus('saving', t('audiocheck', 'Saving…'));
				AudioCheckApi.put('/apps/audiocheck/api/prefs', payload)
					.then((r) => {
						if (seq !== saveSeq || !form.isConnected) {
							return;
						}
						...
						AudioCheckPlayer.setVolumePercent(payload.defaultVolume, { persist: false });
						setStatus('saved', t('audiocheck', 'Saved.'));
						if (window.AudioCheckMessaging && AudioCheckMessaging.announce) {
							AudioCheckMessaging.announce(t('audiocheck', 'Saved.'));
						}
					})
					.catch((err) => {
						if (seq !== saveSeq || !form.isConnected) {
						...
						const msg = err.message || t('audiocheck', 'Request failed.');
						setStatus('error', msg);
						AudioCheckMessaging.toast(msg, 'error');
					});
			}
			...
			function scheduleSave() {
				...
				saveTimer = window.setTimeout(persistPrefs, 280);
			}
			...
			AudioCheckApi.get('/apps/audiocheck/api/prefs').then((r) => {
				...
				hydrated = true;
				setStatus('idle', t('audiocheck', 'Changes save automatically.'));
			}).catch((err) => {
				const msg = err.message || t('audiocheck', 'Request failed.');
				body.replaceChildren(C.loadErrorState(
					t('audiocheck', 'Could not load settings'),
					msg,
					() => AudioCheckRouter.navigate('settings', {}, true),
					{ icon: 'settings' },
				));
				AudioCheckMessaging.toast(msg, 'error');
			});

			const helpDetails = C.el('details', { className: 'ac-disclosure' });
			const helpSummary = C.el('summary', {
				className: 'ac-disclosure__summary',
				text: t('audiocheck', 'Playback help and shortcuts'),
			});
			const helpBody = C.el('div', { className: 'ac-disclosure__body' });
			...
			helpBody.appendChild(onScreen);
			helpBody.appendChild(shortcuts);
