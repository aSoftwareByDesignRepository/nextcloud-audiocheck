 ...
 * @var \OCP\IL10N $l
 * @var \OCA\AudioCheck\Support\AppFeedbackLinks $appFeedbackLinks optional; constructed when omitted
...
$l = $l ?? (\OCP\Util::getL10N('audiocheck'));
...
if (!isset($appFeedbackLinks) || !$appFeedbackLinks instanceof AppFeedbackLinks) {
	$appFeedbackLinks = new AppFeedbackLinks('audiocheck', 'AudioCheck', $version);
}
$pageUrl = '';
...
>
	<div class="<?php p($prefix); ?>-nav-footer__popover">
		<button
			type="button"
			class="<?php p($prefix); ?>-nav-footer__trigger ac-nav__link"
			aria-expanded="false"
			aria-controls="<?php p($menuId); ?>"
			aria-haspopup="true"
		>
			<span class="ac-nav__icon" aria-hidden="true"><?php
				print_unescaped(IconCatalog::render('info'));
			?></span>
			<span class="ac-nav__label">
				<span class="ac-nav__name"><?php p($l->t('Help & Feedback')); ?></span>
			</span>
		</button>
		<ul
