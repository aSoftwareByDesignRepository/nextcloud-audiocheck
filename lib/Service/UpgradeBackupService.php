...
/**
 * Snapshot and restore AudioCheck data before app updates.
 *
...
use DateTimeZone;
use OCA\AudioCheck\Exception\UpgradeBackupException;
...
{
	private const LOCK_KEY = 'audiocheck-upgrade-backup';
	...
	{
		return $this->runExclusive('AudioCheck upgrade backup', function () use ($reason, $rotate, $rotationExcludeIds): array {
	...
	private function doCreateSnapshot(string $reason, bool $rotate, array $rotationExcludeIds): array
	{
		...
		if (!$this->hasDataToBackup()) {
			throw new UpgradeBackupException('No AudioCheck tables exist yet; nothing to back up.');
		}
		...
		try {
			...
			if ($backedUpTables === []) {
				throw new UpgradeBackupException('No AudioCheck tables exist on this instance; nothing to back up.');
			}
		...
		$this->logger->info('AudioCheck: created pre-update backup snapshot', [
	...
	public function restoreSnapshot(string $snapshotId, bool $createSafetySnapshot = true): void
	{
		...
		$this->runExclusive('AudioCheck upgrade restore', function () use ($snapshotId, $folder, $tableManifest, $createSafetySnapshot): void {
	...
	private function doRestoreSnapshot(
		...
		try {
			...
			$this->db->rollBack();
			throw new UpgradeBackupException('AudioCheck restore failed; database changes were rolled back.', 0, $e);
		} finally {
			if ($fkChecksDisabled) {
		...
		try {
			...
			throw new UpgradeBackupException(
				'AudioCheck database was restored but app config or app data folders failed. '
		...
		$this->logger->warning('AudioCheck: restored data from upgrade backup snapshot', [
			'app' => UpgradeBackupCatalog::APP_ID,
	...
	private function getAppDataRootFolder(): Folder
	{
		...
		if (!$node instanceof Folder) {
			throw new UpgradeBackupException('AudioCheck app data root is not a folder.');
		}
	...
	private function getBackupRootFolder(): Folder
	{
		...
		if (!$node instanceof Folder) {
			throw new UpgradeBackupException('AudioCheck upgrade backup root is not a folder.');
		}
	...
	private function runExclusive(string $label, callable $callback)
	{
		try {
			...
			throw new UpgradeBackupException(
				'Another AudioCheck backup or restore is already in progress.',
				0,
				$e,
			);
		}
