...
/**
 * List, create, or restore AudioCheck upgrade backups (CLI operator interface).
 *
...
use OCA\AudioCheck\Service\UpgradeBackupCatalog;
use OCA\AudioCheck\Service\UpgradeBackupIntegrity;
...
{
	private const APP_LABEL = 'AudioCheck';
	...
	{
		$this
			->setName('audiocheck:upgrade-backup')
			->setDescription('Manage AudioCheck snapshots taken before app updates.')
			->setHelp(
				<<<'HELP'
...
<info>create</info> — manual snapshot before risky changes:
  <comment>occ audiocheck:upgrade-backup create --reason="before manual fix"</comment>

<info>restore</info> — destructive; requires <comment>--force</comment> and snapshot id:
  <comment>occ audiocheck:upgrade-backup restore --latest --force</comment>
  <comment>occ audiocheck:upgrade-backup restore --id=YYYYMMDDTHHMMSSZ-hex --force</comment>
...
HELP
			...
			->addOption('force', null, InputOption::VALUE_NONE, 'Required for restore — overwrites current AudioCheck data')
	...
	private function runList(SymfonyStyle $io): int
	{
		...
		if ($snapshots === []) {
			...
			$io->note('Create one manually: occ audiocheck:upgrade-backup create --reason="before change"');
	...
	private function runRestore(InputInterface $input, SymfonyStyle $io): int
	{
		...
		if (!$input->getOption('force')) {
			$io->error([
				'Restore overwrites all current AudioCheck database rows, app config, and configured app-data folders.',
		...
		if ($snapshotId === '') {
			$io->error([
				...
				'  occ audiocheck:upgrade-backup restore --latest --force',
				'  occ audiocheck:upgrade-backup restore --id=<snapshot-id> --force',
				'Run `occ audiocheck:upgrade-backup list` to see snapshot ids.',
			]);
			return Command::FAILURE;
		}
