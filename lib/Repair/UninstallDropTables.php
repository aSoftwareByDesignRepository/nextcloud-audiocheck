...
final class UninstallDropTables implements IRepairStep
{
	...
	public function run(IOutput $output): void
	{
		...
		$output->info(
			'audiocheck: preserving data on disable. '
	...
	private function dropAllTablesAndMetadata(IOutput $output): void
	{
		...
		$output->info(sprintf(
			'audiocheck: dropped %d of %d table(s); removed %d migration row(s), app config, and upgrade-backup snapshots.',
			$dropped,
			count(self::TABLES),
			$migrationsRemoved,
		));
	...
	private function purgeUpgradeBackupSnapshots(IOutput $output): void
	{
		...
		$node->delete();
		$output->info('audiocheck: removed upgrade-backup snapshots from app data.');
	}
}
