...
/**
 * Pre-migration repair step: snapshot AudioCheck data before schema migrations run.
 *
...
final class BackupBeforeUpdate implements IRepairStep
{
	...
	{
		return 'Back up AudioCheck data before update migrations';
	}

	public function run(IOutput $output): void
	{
		if (!$this->backupService->hasDataToBackup()) {
			$output->info('AudioCheck: no existing tables to back up (fresh install); skipping pre-update snapshot.');
			return;
		}
		...
		try {
			...
			$output->warning('AudioCheck: pre-update backup failed: ' . $e->getMessage());
			throw $e;
		}

		$tableCount = count($result['manifest']['tables'] ?? []);
		$output->info(sprintf(
			'AudioCheck: pre-update backup created (%s, %d table(s)). '
			. 'Restore with `occ audiocheck:upgrade-backup restore --latest --force` if needed.',
			$result['id'],
			$tableCount,
		));
	}
