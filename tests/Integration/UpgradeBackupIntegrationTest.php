...
final class UpgradeBackupIntegrationTest extends TestCase
{
	...
	{
		if (!$this->db->tableExists('ac_libraries')) {
			self::markTestSkipped('AudioCheck tables not present in this instance.');
		}

		$before = $this->countRows('ac_libraries');
		...
		$this->db->getQueryBuilder()
			->delete('ac_libraries')
			->executeStatement();
		self::assertSame(0, $this->countRows('ac_libraries'));

		$this->backupService->restoreSnapshot($snapshotId, false);
		self::assertSame($before, $this->countRows('ac_libraries'));
	}
