#!/usr/bin/env node
/**
 * Behavioural tests for AudioCheck app-feedback.js (mailto + toast hook).
 *
...
function boot(extra) {
	...
	const config = {
		appId: 'audiocheck',
		appDisplayName: 'AudioCheck',
		appVersion: '1.0.0',
		feedbackEmail: 'dev@software-by-design.de',
		githubIssuesUrl: 'https://github.com/aSoftwareByDesignRepository/nextcloud-audiocheck/issues',
		cssPrefix: 'ac',
		...(extra || {}),
	};
	const cfgEl = {
	...
	nodes.set('ac-app-feedback-config', cfgEl);
	...
	nodes.set('ac-feedback-problem', problem);
	nodes.set('ac-feedback-idea', idea);

	const trigger = {
		className: 'ac-nav-footer__trigger',
		getAttribute(k) {
			return this[k] || null;
		},
		setAttribute(k, v) {
	...
	trigger['aria-controls'] = 'ac-feedback-menu';
	const menu = {
		id: 'ac-feedback-menu',
		hidden: true,
		contains() {
	...
	nodes.set('ac-feedback-menu', menu);
	doc.querySelector = (sel) => {
		if (sel === '.ac-nav-footer__trigger') {
			return trigger;
		}
		return null;
	};
	doc.getElementById = (id) => {
		if (id === 'ac-feedback-menu') {
			return menu;
		}
		return nodes.get(id) || null;
	};
	...
	const sandbox = {
		...
		location: {
			...
			pathname: '/apps/audiocheck/today',
			search: '?token=secret&keep=1',
			href: 'https://cloud.example/apps/audiocheck/today?token=secret&keep=1',
		},
...
function main() {
	...
	const clean = sandbox.SbdAppFeedback.sanitizePageUrl(
		'https://cloud.example/apps/audiocheck/x?token=abc&keep=1&password=no'
	);
	...
	const upgradeAck = sandbox.SbdAppFeedback.sanitizePageUrl(
		'/apps/audiocheck/?IKnowThatThisIsABigInstanceAndTheUpdateRequestCouldRunIntoATimeoutAndHowToRestoreABackup=IAmSuperSureToDoThis'
	);
	assert(upgradeAck === '/apps/audiocheck/', 'strips Nextcloud web-upgrade ack query key');
	...
	process.stdout.write('app-feedback.test.js OK (audiocheck)\n');
}
