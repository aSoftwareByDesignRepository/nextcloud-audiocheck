...
use OCA\AudioCheck\Support\AppFeedbackLinks;
...
final class AppFeedbackLinksTest extends TestCase
{
	...
	{
		$links = new AppFeedbackLinks('audiocheck', 'AudioCheck', '1.2.3');
		$en = $links->problemMailto(['pageUrl' => '/apps/audiocheck/today'], 'en');
		$de = $links->problemMailto(['pageUrl' => '/apps/audiocheck/today'], 'de');
		self::assertStringStartsWith('mailto:dev@software-by-design.de?subject=', $en);
		self::assertStringContainsString(rawurlencode('AudioCheck: Problem report'), $en);
		self::assertStringContainsString(rawurlencode('AudioCheck: Fehlermeldung'), $de);
	...
	{
		$links = new AppFeedbackLinks('audiocheck', 'AudioCheck');
		$href = $links->ideaMailto([], 'de');
		self::assertStringContainsString(rawurlencode('AudioCheck: Feedback'), $href);
	}
	...
	public function testGithubIssuesUrlIsPublicVendorRepo(): void
	{
		$links = new AppFeedbackLinks('audiocheck', 'AudioCheck');
		self::assertSame(
			'https://github.com/aSoftwareByDesignRepository/nextcloud-audiocheck/issues',
	...
	{
		$links = new AppFeedbackLinks('audiocheck', 'AudioCheck');
		$clean = $links->sanitizePageUrl('https://cloud.example/apps/audiocheck/x?token=abc&keep=1&password=no');
		...
		$keep = $links->sanitizePageUrl('https://cloud.example/apps/audiocheck/list?keep=1&token=abc');
	...
	{
		$links = new AppFeedbackLinks('audiocheck', 'AudioCheck');
		$noisy = '/apps/audiocheck/?IKnowThatThisIsABigInstanceAndTheUpdateRequestCouldRunIntoATimeoutAndHowToRestoreABackup=IAmSuperSureToDoThis';
		self::assertSame('/apps/audiocheck/', $links->sanitizePageUrl($noisy));
	}

	public function testMailtoBodyOmitsIdentityAndRejectsUnsafeErrorCodes(): void
	{
		$links = new AppFeedbackLinks('audiocheck', 'AudioCheck', '9.9.9');
		$href = $links->problemMailto([
			'errorCode' => 'CONFLICT',
			'pageUrl' => '/apps/audiocheck/',
		], 'en');
		...
		self::assertStringContainsString('App id: audiocheck', $body);
	...
	public function testRejectsUnsafeDisplayName(): void
	{
		...
		new AppFeedbackLinks('audiocheck', "Evil\r\nBcc: attacker@example.com");
	}

	public function testForLocalePayload(): void
	{
		$links = new AppFeedbackLinks('audiocheck', 'AudioCheck', '1.0.0');
		$payload = $links->forLocale('de', ['pageUrl' => '/apps/audiocheck/']);
		...
		self::assertSame('audiocheck', $payload['appId']);
	}
}
