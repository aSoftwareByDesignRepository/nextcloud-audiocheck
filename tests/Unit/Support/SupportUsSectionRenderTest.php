...
use OCA\AudioCheck\Support\SupportUsLinks;
...
final class SupportUsSectionRenderTest extends TestCase {
	...
	public function testRenderEscapesDisplayNameAndOmitsMobileWithoutFlag(): void {
		$html = $this->renderSection(
			new SupportUsLinks('AudioCheck', false, null),
			'en'
		);
		...
		self::assertStringContainsString(rawurlencode('AudioCheck: partner / care retainer'), $html);
	...
	public function testRenderIncludesMobileLicenseWhenConfigured(): void {
		...
		self::assertStringContainsString('href="/apps/audiocheck/admin/license"', $html);
		self::assertStringContainsString(rawurlencode('AudioCheck: Partner / Care Retainer'), $html);
